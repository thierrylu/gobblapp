...
{
    if (index >= 0 && index <= _imageViews.count-1 && _imageViews.count > 0) {
        UIImageView *imv = (UIImageView *)[_imageViews objectAtIndex:index];
        [UIView animateWithDuration:0.5 delay:0.0 options:UIViewAnimationOptionCurveEaseOut animations:^{
            imv.frame = CGRectMake(320.0, imv.frame.origin.y, imv.frame.size.width, imv.frame.size.height);
            imv.alpha = 0.0;
...
- (void)removeImageAtIndex2:(NSInteger)index animated:(BOOL)animated
{
    if (index >= 0 && index <= _imageViews.count-1 && _imageViews.count > 0) {
        UIImageView *imv = (UIImageView *)[_imageViews objectAtIndex:index];
        [UIView animateWithDuration:0.5 delay:0.0 options:UIViewAnimationOptionCurveEaseOut animations:^{
...
//    }];
    ...
    [_delegate didSelectedOnGalleryView:[_images lastObject]];
}
