...
- (void)setupView:(float)scale {
	self.clipsToBounds = YES;
	currentIdx = -1;
	star = [[UIImage imageNamed:@"big_star_empty.png"] retain];
	highlightedStar = [[UIImage imageNamed:@"big_star_full.png"] retain];

	for (int i=0; i<numberOfStars; i++) {
		DLStarView *v = [[DLStarView alloc] initWithDefault:self.star highlighted:self.highlightedStar position:i allowFractions:isFractionalRatingEnabled andScale:scale];
		[self addSubview:v];
		[v release];
	}
}
...
- (id)initWithFrame:(CGRect)frame andStars:(NSUInteger)_numberOfStars isFractional:(BOOL)isFract andScale:(float)scale{
	self = [super initWithFrame:frame];
	if (self) {
