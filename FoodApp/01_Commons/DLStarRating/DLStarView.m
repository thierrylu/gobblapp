...
- (id)initWithDefault:(UIImage*)star highlighted:(UIImage*)highlightedStar position:(int)index allowFractions:(BOOL)fractions andScale:(float)scaleStar{
	self = [super initWithFrame:CGRectZero];
    
    _scaleStar = scaleStar;
	...
	if (self) {
        ...
        self.frame = CGRectMake((star.size.width*index), 0, star.size.width/scaleStar, star.size.height/scaleStar+kEdgeInsetBottom);
...
- (void)centerIn:(CGRect)_frame with:(int)numberOfStars {
	...
	if (_scaleStar >= 1.2 && _scaleStar <= 1.3) {
        self.frame = CGRectMake((size.width*self.tag) + gapToApply + (self.tag-1) * 10, newY, size.width, size.height);
    }
	else{
        self.frame = CGRectMake((size.width*self.tag) + gapToApply + (self.tag-1) * 3, newY, size.width, size.height);
    }
}
